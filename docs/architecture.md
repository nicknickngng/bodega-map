...
- **Marker stability + memory (fixed 2026-06-14):** On device the map originally (a) crashed after ~30 s of panning, from rebuilding 150–200 custom-`<View>` markers as native iOS annotations on every region change, and (b) showed inconsistent/blank clusters from an unsequenced async race plus a `react-native-maps` `tracksViewChanges` quirk. Marker churn, not DB load, was the bottleneck. Fixes shipped: **debounced auto-reload** (400 ms after the viewport settles), **stale-response sequencing** (a `requestId` ref; only the latest result is applied), **stable marker keys** (the RPC now returns grid-cell indices `gx`/`gy`; markers are keyed `gx:gy` rather than the drifting centroid) **+ `tracksViewChanges` handling** (on for ~500 ms after each update, then off), and a **coarser grid** (`CELLS_ACROSS` 14 → 9). See `decisions.md` and `current-task.md`.
...
- **`bodegas_clusters(min_lat, min_lng, max_lat, max_lng, grid)`** — grid-aggregated clusters (centroid + count per cell, plus grid-cell indices `gx`/`gy` for stable marker keys) for the map. Bounded row count avoids the 1,000-row cap. See the Map section. Defined in `supabase/migrations/0003_clusters_function.sql`, extended with `gx`/`gy` in `0004_clusters_grid_keys.sql`.
